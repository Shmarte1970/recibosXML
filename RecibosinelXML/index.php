...
if (!empty($datosDefectuosos)) {
    echo "<script>('Se encontraron " . count($datosDefectuosos) . " registros con datos incorrectos.');</script>";
} else {
    echo "<script>('Todos los registros están correctos.');</script>";
}
...
function enviarInforme($archivo_txt) {
    if (!file_exists($archivo_txt)) {
        return "Error: El archivo de informe no existe.";
    }
...
function descargarInforme($archivo_txt) {
    $directorio = 'C:\\Informes\\';
    $ruta_archivo = $directorio . $archivo_txt;
    
    // Crear el directorio si no existe
    if (!file_exists($directorio)) {
        if (!mkdir($directorio, 0755, true)) {
            die('Fallo al crear el directorio de informes...');
        }
    }
...
if (isset($_POST['generateReport'])) {
    $archivo_txt = generarInformeRecibosDefectuosos($datosDefectuosos);
    $resultado = enviarInforme($archivo_txt);
    echo "<script>alert('$resultado');</script>";
    
    // Añadir enlace para descargar el informe
    echo "<a href='descargarInforme.php?filename=" . urlencode($archivo_txt) . "'>Descargar Informe</a>";
}
