...
// Consulta a la tabla temporary para obtener los primeros 20 registros
$listQuery = "SELECT nom, NUMFACTURA, IMPORT, DATE, DATEVENCIMENT FROM temporary";
$resultList = $covellog->query($listQuery);

$dataList = array();

if ($resultList->num_rows > 0) {
    // Almacenar los datos obtenidos en el array $dataList
    while ($row = $resultList->fetch_assoc()) {
        $dataList[] = $row;
    }
} else {
    echo "<script>console.log('No se encontraron registros en temporary.');</script>";
}
