...
pausa();
echo 'Tabla Temporary vacia';

// Consulta a la base de datos
$sql = "SELECT * FROM rebut where DATA >= ? Limit 1";
...
pausa();
echo 'Vamos a comprobar que los registros no tiene errores';
// Verificación de campos
$verificationSql = "SELECT * FROM temporary WHERE 
    compte IS NULL OR LENGTH(compte) != 24 OR 
    cif IS NULL OR LENGTH(cif) < 9 OR 
    numfactura IS NULL OR 
    numrebut IS NULL OR 
    import IS NULL OR 
    date IS NULL OR 
    datevenciment IS NULL OR 
    nom IS NULL";
$verificationResult = $covellog->query($verificationSql);

if ($verificationResult->num_rows > 0) {
    echo "\nSe encontraron registros con datos incorrectos:\n";
    while ($row = $verificationResult->fetch_assoc()) {
        echo "ID: " . $row['id'] . "\n";
        echo "numfactura: " . ($row['numfactura'] ?: 'ERROR - Campo vacío') . "\n";
        echo "numrebut: " . ($row['numrebut'] ?: 'ERROR - Campo vacío') . "\n";
        echo "import: " . ($row['import'] ?: 'ERROR - Campo vacío') . "\n";
        echo "date: " . ($row['date'] ?: 'ERROR - Campo vacío') . "\n";
        echo "datevenciment: " . ($row['datevenciment'] ?: 'ERROR - Campo vacío') . "\n";
        echo "compte: " . ($row['compte'] ?: 'ERROR - Campo vacío') . 
             (strlen($row['compte']) != 24 ? " (ERROR - Longitud incorrecta: " . strlen($row['compte']) . ")" : "") . "\n";
        echo "nom: " . ($row['nom'] ?: 'ERROR - Campo vacío') . "\n";
        echo "cif: " . ($row['cif'] ?: 'ERROR - Campo vacío') . 
             (strlen($row['cif']) < 11 ? " (ERROR - Longitud incorrecta: " . strlen($row['cif']) . ")" : "") . "\n";
        echo "----------------------\n";
    }
    
    $decision = readline("Se encontraron registros con datos incorrectos. ¿Desea continuar? (s/n): ");
    if (strtolower($decision) !== 's') {
        die("Proceso terminado por el usuario debido a datos incorrectos.");
    }
} else {
    echo "\nTodos los registros en la tabla temporary están correctos.\n";
}
...
foreach ($data as $indice) {
    ...
    $pmtinf->addChild('ReqdColltnDt', htmlspecialchars($formattedDate));
    ...
    $orgId = $id->addChild('OrgId');
    $othr = $orgId->addChild('Othr');
    $othr->addChild('Id', 'ES1300B65767030'); // (PRM identificador Facilitado por la entidad )
    ...
    $instrId = $pmtId->addChild('InstrId',htmlspecialchars($indice['numfactura']));
    ...
    $orgId = $id->addChild('OrgId');
    $othr = $orgId->addChild('Othr');
    $othr->addChild('Id', htmlspecialchars('ES1300B65767030'));
    $schmnm = $othr->addchild('SchmeNm');    
    $prtry =$schmnm->addchild('Prtry','SEPA');
    ...
    $ctry = $pstladr->addChild('Ctry','ES');
    $ctry = $pstladr->addChild('AdrLine','No es necesario la Direccion');
    $id = $dbtr->addChild('Id');
    $orgId = $id->addChild('OrgId');
    $othr = $orgId->addChild('Othr');
